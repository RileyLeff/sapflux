...
pub fn process_multi_sensor_format(
    ...
    if lines.len() < 5 {
        return Err(PipelineError::Processing(
            "Multi-sensor file has fewer than 5 lines, cannot process.".to_string(),
        ));
    }
    ...
    // --- FIX IS HERE ---
    // 1. Read the CSV without trying to name or select columns. It will get default names.
    ...
    // 2. Get the temporary default names and rename them to the correct headers.
    let temp_names: Vec<String> = df_with_temp_names.get_column_names().iter().map(|s| s.to_string()).collect();
    let df = df_with_temp_names
        .lazy()
        .rename(&temp_names, &headers, true)
        .collect()?;

    let sensor_prefixes: Vec<String> = headers
        .iter()
    ...
    if sensor_prefixes.is_empty() {
        return Err(PipelineError::Processing("No sensor data columns found in multi-sensor file.".to_string()));
    }
    ...
    for prefix in sensor_prefixes {
        ...
        for (source_suffix, target_name) in &column_mapping {
            ...
            select_exprs.push(
                 when(col(&full_col_name).cast(DataType::String).eq(lit("NAN")))
                    .then(lit(NULL))
                    .otherwise(
                        when(col(&full_col_name).eq(lit(-99.0)))
                           .then(lit(NULL))
                           .otherwise(col(&full_col_name))
                    )
                    .alias(*target_name) // Your dereference fix was correct!
            );
        }
